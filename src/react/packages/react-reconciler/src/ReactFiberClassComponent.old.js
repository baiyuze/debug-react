...
const classComponentUpdater = {
  ...
  enqueueSetState(inst, payload, callback) {
    ...
    // 计算出update的优先级，如果是state的更新，则优先级为
    // 0b0000000000000000000000000000001;
    // 优先级为1车道为1
    // 比较fiber的mode和ConcurrentMode的位与运算，如果为0，返回
    // 获取当前更新的lane,点击事件为1---> 0b0000001;
    const lane = requestUpdateLane(fiber);
    // 创建update更新函数，传入eventTime和lane
...
function callComponentWillMount(workInProgress, instance) {
  ...
  console.log(instance, workInProgress, 'instance======callComponentWillMount');
...
function mountClassInstance(
  ...
  // 初始化更新队列，生成对列 workInProgress 是fiber
  // 初始化context
  initializeUpdateQueue(workInProgress);
  // ctor /// 组件
  const contextType = ctor.contextType;
  // const MyContext = React.createContext(defaultValue);
  // 这里的contextType是指通过React.createContext进行创建得来的
  if (typeof contextType === 'object' && contextType !== null) {
    instance.context = readContext(contextType);
  } else if (disableLegacyContext) {
    instance.context = emptyContextObject;
  } else {
    // contextType 为undefined时，会走这里，条件不满足
    // 获取未被屏蔽的上下文,获取他的默认值
