...
export function requestEventTime() {
  ...
  // 我们不在里面，所以我们可能正在浏览器事件中。
  if (currentEventTime !== NoTimestamp) {
    // 对所有更新使用相同的开始时间，直到再次输入React。
...
export function scheduleUpdateOnFiber(
  ...
  // 标记从fiber到root的update lane
  // 合并fiber的么个fiberNode的lanes，如果有workInPregess的alternat，也同步更新
...
// on a fiber.
//这被分解成一个单独的函数，这样我们就可以用挂起标记一个fiber
//不将其视为源于事件的典型更新而工作；
//例如，重试悬念边界不是更新，但它确实schedule work
//在fiber上。
function markUpdateLaneFromFiberToRoot(
  sourceFiber: Fiber,
  lane: Lane,
): FiberRoot | null {
  // Update the source fiber's lanes//rootFiber
  // 合并lanes多车道
  ...
  let node = sourceFiber;//当前节点
  let parent = sourceFiber.return;//父节点
  // 从下往上执行...current -> parent
  while (parent !== null) {
    // 合并子优先级
...
export function isInterleavedUpdate(fiber: Fiber, lane: Lane) {
  return (
    ...
    //TODO:通过与fiber所属的根进行比较，稍微优化。
    //需要一些重构。不过没什么大不了的，因为这对我来说很少见
    ...
    //如果这是渲染阶段更新（即不安全的\u componentWillReceiveProps），
    //那就不要把它当作interleaved更新。这种模式是
    //伴随着一个警告，但我们还没有完全反对它。我们可以
    //启用DeferRenderPaseUpdateToNextBatch标志后删除。
    (deferRenderPhaseUpdateToNextBatch ||
      (executionContext & RenderContext) === NoContext)
  );
}
...
function pushInteractions(root) {
  if (enableSchedulerTracing) {//启用调度程序跟踪
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  const current = unitOfWork.alternate;//fiberRoot
...
function commitRootImpl(root, renderPriorityLevel) {
  console.log(root, renderPriorityLevel, 'renderPriorityLevel');
  do {
...
//将当前的交互集存储在FiberRoot上有几个原因：
//我们可以在performConcurrentWorkinRoot（）这样的热门函数中重用它
//不用重新计算。我们还将在commitwork（）中使用它
//传递给任何Profiler onRender（）钩子。这也为DevTools提供了
//调用onCommitRoot（）钩子时访问它的方法。
